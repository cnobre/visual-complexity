...
df %>% 
  ...
  coord_flip() +
    facet_grid(task ~ complexity )


df %>% 
  filter(measure=="answer") %>% 
  mutate( value = as.numeric( as.character(value)) ) %>% 
  ggplot( aes(x=condition, y=value) ) +
    geom_jitter( alpha = 0.2, width=0.1, height=0.3 ) +
   ylim(0, 50)+
...
df %>% 
  filter(task =="Complexity" | task == "Understanding"  | task == "Visual Literacy" | task == "Animation 1" | task == "Animation 2") %>% 
  mutate( value = as.numeric( as.character(value)) ) %>% 
  ggplot( aes(x=condition, y=value) ) +
    geom_jitter( alpha = 0.2, width=0.1, height=0.3 ) + 
    stat_summary(fun.data = "mean_cl_boot", colour = "red", size = 0.5, position = position_nudge(x=0.25, y=0), alpha=0.5) +
...
# Knowledge
sd_knowledge = df_acc %>% 
  filter(task == "Knowledge") %>% 
  summarise(
    stdev = sd(value)
  )

m_static_knowledge = df_acc %>% 
  filter(task == "Knowledge" & condition == "static") %>% 
  summarise(
    mean = mean(value)
  )

m_animated_knowledge = df_acc %>% 
  filter(task == "Knowledge" & condition == "animated") %>% 
  summarise(
    mean = mean(value)
  )
...
# Application
sd_application = df_acc %>% 
  filter(task == "Application") %>% 
  summarise(
    stdev = sd(value)
  )

m_static_application = df_acc %>% 
  filter(task == "Application" & condition == "static") %>% 
  summarise(
    mean = mean(value)
  )

m_animated_application = df_acc %>% 
  filter(task == "Application" & condition == "animated") %>% 
  summarise(
    mean = mean(value)
  )
...
# Knowledge 2
sd_knowledge2 = df_acc %>% 
  filter(task == "Knowledge 2") %>% 
  summarise(
    stdev = sd(value)
  )

m_static_knowledge2 = df_acc %>% 
  filter(task == "Knowledge 2" & condition == "static") %>% 
  summarise(
    mean = mean(value)
  )

m_animated_knowledge2 = df_acc %>% 
  filter(task == "Knowledge 2" & condition == "animated") %>% 
  summarise(
    mean = mean(value)
  )
...
powerAnalysisGraph(m_static_application[,1], m_animated_application[,1], sd_application[,1])

# Knowledge 2 Questions
powerAnalysisGraph(m_static_knowledge2[,1], m_animated_knowledge2[,1], sd_knowledge2[,1])
